...
def scenario():
    ...
    problem = {'type': 'slack', 'penalize_violations': True,
               'algorithm':'gd', 'name':'slack_penalized'}
    ...
    c1 = 10      # linear penalty
    c2 = 1      # quadratic penalty
    ...
    alg['rho'] = 0.1
    alg['eta'] = 1
    alg['alpha'] = lambda k,J : 0.1*log(k+1)/(k+1)
    ...
    var_out['eps'] = range((n_x+n_u)*N,(n_x+n_u)*N+2*n_x*N)
    # var_out['x'] = range(0,n_x*N)
